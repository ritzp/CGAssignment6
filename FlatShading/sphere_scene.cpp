...
std::vector<Vector3> triangleColors;
...
void calculate_triangle_colors() {
    triangleColors.resize(gNumTriangles);
    ...
    for (int i = 0; i < gNumTriangles; ++i) {
        ...
        triangleColors[i] = shading(normal, lightPos, viewPos, center);
    }
}
...
void rasterize_triangles() {
    ...
    for (int i = 0; i < gNumTriangles; ++i) {
        ...
        Vector3 flat_color = triangleColors[i];
            ...
            for (int x = minX; x <= maxX; ++x) {
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                    ...
                    if (z < depthBuffer[x][y]) {
                        ...
                        OutputImage[index + 0] = flat_color.x; // R
                        OutputImage[index + 1] = flat_color.y; // G
                        OutputImage[index + 2] = flat_color.z; // B
                    }
                }
            }
        }
