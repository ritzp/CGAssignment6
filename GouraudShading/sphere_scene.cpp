...
public:
    float x, y, z, w = 1.0f;
    Vector3 color; // Color for Gouraud shading
};
            ...
            for (int x = minX; x <= maxX; ++x) {
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                    ...
                    if (z < depthBuffer[x][y]) {
                        ...
                        Vector3 interpolatedColor = c0 * w0 + c1 * w1 + c2 * w2;
                        int index = (y * WIDTH + x) * 3;
                        OutputImage[index + 0] = interpolatedColor.x;
                        OutputImage[index + 1] = interpolatedColor.y;
                        OutputImage[index + 2] = interpolatedColor.z;
                    }
                }
            }
        }
